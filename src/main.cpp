...
// Using rotation=0 for standard portrait orientation
...
// Define touch buttons for PORTRAIT (240x320)
TouchButton btnStartPause = {10, 260, 70, 50, "START", TFT_GREEN, TFT_BLACK};
TouchButton btnNext = {90, 260, 70, 50, "NEXT", TFT_BLUE, TFT_WHITE};
TouchButton btnPrev = {170, 260, 60, 50, "PREV", TFT_ORANGE, TFT_BLACK};
TouchButton btnConfig = {200, 5, 30, 30, "CFG", TFT_DARKGREY, TFT_WHITE}; // Config button top right
TouchButton btnCancelConfig = {60, 250, 120, 50, "CANCEL", TFT_RED, TFT_WHITE}; // Cancel button for config mode
...
void loadTimerState() {
  ...
  if (preferences.isKey("state_round")) {
    ...
    // Draw Resume and Start Fresh buttons
    tft.fillRoundRect(30, 200, 80, 50, 8, TFT_GREEN);
    tft.fillRoundRect(130, 200, 80, 50, 8, TFT_RED);
    
    tft.setTextSize(2);
    tft.setTextColor(TFT_BLACK);
    tft.setCursor(37, 215);
    tft.println("RESUME");
    tft.setTextColor(TFT_WHITE);
    ...
    while (!chose && (millis() - startWait < 10000)) {
      if (ts.touched()) {
        ...
        // Check Resume button (left)
        if (x >= 30 && x <= 110 && y >= 200 && y <= 250) {
          playButtonBeep();
          // Resume
        ...
        // Check Start Fresh button (right)
        else if (x >= 130 && x <= 210 && y >= 200 && y <= 250) {
          playButtonBeep();
          // Start fresh
          clearTimerState();
          currentRound = 0;
...
void drawTimerDisplay() {
  ...
  // PORTRAIT LAYOUT (240x320) - vertical stacking
  
  // Display round info (top)
  tft.setTextSize(2);
  ...
  // Draw time remaining (large, centered)
  ...
  // Top right area - show gear when paused, or next round info when running
  if (!timerRunning) {
...
void handleTouch() {
  ...
  // Map touch coordinates for PORTRAIT rotation 0 (240x320)
  // Standard CYD calibration for rotation 0
  int x = map(p.x, 200, 3700, 0, 240);  // Map to width 240
  int y = map(p.y, 240, 3800, 0, 320);  // Map to height 320
  ...
  if (isTouchInButton(x, y, btnConfig)) {
    ...
    // Draw YES/NO buttons
    tft.fillRoundRect(30, 220, 80, 50, 8, TFT_GREEN);
    tft.fillRoundRect(130, 220, 80, 50, 8, TFT_RED);
    
    tft.setTextSize(2);
    tft.setTextColor(TFT_BLACK);
    tft.setCursor(50, 237);
    tft.println("YES");
    tft.setTextColor(TFT_WHITE);
    ...
    while (!confirmed && !cancelled && (millis() - confirmStart < 10000)) {
      if (ts.touched()) {
        ...
        if (x2 >= 30 && x2 <= 110 && y2 >= 220 && y2 <= 270) {
          confirmed = true;
          Serial.println("Config mode confirmed");
        }
        // Check NO button (right)
        else if (x2 >= 130 && x2 <= 210 && y2 >= 220 && y2 <= 270) {
          cancelled = true;
    ...
    if (currentRound < totalRounds - 1) {
      ...
      while (!confirmed && !cancelled && (millis() - confirmStart < 10000)) {
        if (ts.touched()) {
          ...
          // Check YES button (left)
          if (x2 >= 30 && x2 <= 110 && y2 >= 200 && y2 <= 250) {
            confirmed = true;
            playButtonBeep();
            Serial.println("Next round confirmed");
          }
          // Check NO button (right)
          else if (x2 >= 130 && x2 <= 210 && y2 >= 200 && y2 <= 250) {
            cancelled = true;
            playButtonBeep();
...
void setup() {
  ...
  // Use rotation 0 for PORTRAIT orientation
  tft.setRotation(0);
  Serial.print("Display rotation 0 (portrait): ");
  ...
  ts.setRotation(0); // Match display rotation (portrait)
  Serial.println("✓ Touch rotation set to 0 (portrait)");
  ...
  if (configMode) {
    ...
    if (ts.touched() && (currentTime - lastConfigTouch > TOUCH_DEBOUNCE)) {
      ...
      if (isTouchInButton(x, y, btnCancelConfig)) {
        ...
        // If timeout, return to config mode
